...
export default class SimpleLocalize extends BaseProvider<SimpleLocalizeConfig> {
  ...
  public async getTranslations(
    ...
    const data = await this.get<any>(
      `https://cdn.simplelocalize.io/${this.config.projectId}/_latest/${language}`
    )
  ...
  private async assureLanguageExists() {
    ...
    while (nonExistingCodes.length > 0) {
      ...
      logger.info(
        `Going add language "${key}" to project "${this.config.projectName}"`
      )
  ...
  private async getToken(): Promise<string> {
    ...
    if (!apiKey) {
      ...
      throw new Error(
        `No API key defined, please set "${
          this.config.tokenFrom || 'NX_EXTEND_SIMPLE_LOCALIZE_TOKEN'
        }"!`
      )
    }
  ...
  private getBasicAuth(): string {
    ...
    if (!basicAuth) {
      throw new Error(
        `No basic auth key defined, please set "NX_EXTEND_SIMPLE_BASIC_AUTH"!`
      )
    }
  ...
  private async autoTranslate(language: string) {
    try {
      ...
      if (
        ...
        !DEEPL_SUPPORT_LANGUAGES.includes(language.toUpperCase())
      ) {
        if (this.config.translatorOptions?.fallbackToGooleTranslate) {
          logger.warn(
            `Language "${language}" will use Google Translate as it's not supported by DeepL!`
          )
          translationProvider = 'GOOGLE_TRANSLATE'
        } else {
          logger.warn(
            `Skipping auto translate for "${language}" as it's not support by DeepL!`
          )
          return
        }
      }
      ...
      await this.post(
        '/jobs/auto-translate',
        {
          ...
          deeplFormality:
            this.config?.translatorOptions?.formality ?? undefined,
  ...
  private async get<Data = any>(
    url: string,
    config?: AxiosRequestConfig<Data>,
    isRetry = false
  ) {
    try {
      const response = await this.apiClient.get<Data>(url, config)
  ...
  private async post<Data = any>(
    ...
    try {
      const response = await this.apiClient.post<Data>(url, data, config)
